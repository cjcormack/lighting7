...
**Phase**: 1 — in flight. Backend DB + DTO + route round-trip landed 2026-04-19a. Layer-3 apply integration + stomp switch landed 2026-04-19b. cueEdit sockets, legacy-effect migration, frontend routing, crossfade-weight integration still pending.

**Most recent session**: 2026-04-19b. Wired `applyCue()` and `CueStackManager.activateCueInStack()` into Layer 3: added `FxEngine.setCueAssignments(cueId, …)` / `removeCueAssignments(cueId)` / `clearAllCueAssignments()` tracking per-cue contributions under a `ConcurrentHashMap<Int, List<Assignment>>` and republishing the flat list to `LayerResolver.applyAssignments(...)` on every change. New `Layer3Resolver.Companion.parseAssignmentValue(category, propertyName, value)` parses the DTO's single `value: String` into a typed `PropertyValue` (Slider / Setting / Colour from `parseExtendedColour` / Position from `"pan,tilt"`). New `buildLayer3AssignmentsForCue(fixtures, cueData)` helper in `projectCues.kt` expands group targets to per-member Assignments while also emitting the group-level row (so the specificity rule has work when a fixture-level assignment overrides within the same cue), canonicalises `colour`/`color`/`rgbcolour` to `rgbColour`, and looks up category/composition via `fixture.fixtureProperties`. `applyCue` now publishes the assignments before applying effects so the effect reset pass sees Layer 3 immediately; stomp overlap switched to use the same assignments. `removeEffectsForCue` / `clearAllEffects` / `stop` / `CueStackManager.deactivateStack` all drop the cue's Layer 3 state. Added `FxEngineCueAssignmentsTest` (7 tests), `BuildLayer3AssignmentsForCueTest` (7 tests, exercises real HexFixture + group), and 9 new parser tests in `Layer3ResolverTest`. All 550 tests pass.

**Prior session**: 2026-04-19a — Phase 1 backend foundation. **2026-04-18** — Phase 0 complete. See change log entries.
...
**Next actions**:
1. Manual smoke check: apply a cue with `propertyAssignment(group "front-wash", dimmer=180)` through the frontend cue form; verify fixtures hold 180 with no effect running; start SineWave ADDITIVE on dimmer; confirm the effect wiggles above 180, not above 0; stop cue; confirm release. Not yet verified with a running backend/frontend in this session.
...
| 1 | `CuePropertyAssignment` model + migration; frontend `EditorContext` routing layer | In flight (backend DB + DTO + routes + Layer 3 apply + stomp switch done; frontend + cueEdit sockets + legacy migration + crossfade-weight still pending) |
