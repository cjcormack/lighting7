...
**Phase**: 1 — in flight. Backend DB + DTO + route round-trip landed 2026-04-19a. Layer-3 apply integration + stomp switch landed 2026-04-19b. cueEdit sockets + legacy-effect migration + snapshot-from-live landed 2026-04-19c. Smoke-check + frontend plumbing + cueEdit.setMode/clearAssignment landed 2026-04-19d. Crossfade-weight integration and a **newly-discovered Layer 3 transmit gap** (see §"Known issues") are the remaining Phase 1 items.

**Most recent session**: 2026-04-19d. Smoke-check run against a live backend + frontend, frontend `EditorContext` plumbing landed, and the two easiest cueEdit follow-up messages (`setMode` + `clearAssignment`) implemented. One real bug found in smoke-check — see §"Known issues".

**Prior sessions**: 2026-04-19c — cueEdit core sockets + legacy migration + snapshot-from-live. See change log entries.
...
**Layer 3 transmit gap (discovered 2026-04-19d smoke-check) — blocking Phase 1 exit.** When a cue with `propertyAssignments` but no effects is applied, the stage doesn't paint. `FxEngine.processBeatTick` / `processWallClockTick` early-return when no effects are running (`if (beatEffects.none { it.isRunning }) return`), so the only path that currently composes Layer 3 onto controllers — the effect-reset pass — never runs. Cues that include ≥1 effect work fine because the reset pass then fires every tick and composes `Layer 3 → Layer 4 → Layer 5` into its fallback target. Symptoms observed: a `(group hexes, dimmer=180)` cue applied on a channel previously at 0 leaves the channel at 0; adding a SineWave ADDITIVE makes the channel jump to the 180–220 band immediately (confirming Layer 3 is composed correctly by the reset pass).

The same gap breaks `cueEdit.setProperty` / `cueEdit.setChannel` in **LIVE mode** when the edited cue has no effects: the assignment persists to the DB and `setCueAssignments` updates the `@Volatile` Layer 3 snapshot, but without an effect tick the controller never sees it.

Fix sketch: when Layer 3 is published / unpublished (`FxEngine.setCueAssignments` / `removeCueAssignments` / `clearAllCueAssignments`), do a one-shot walk of affected `(fixtureKey, propertyName)` pairs, compose Layer 3 → Layer 4 → Layer 5 per channel, and emit a `ControllerTransaction`. Mirror of how `updateChannel` (Layer 4) writes both `DirectWriteStore` and `controller.setValue` in the same message handler. Release-on-unpublish needs the layer-below computation already in `FxTarget.fallbackFromDirectWrites`. No failing unit test landed — the test harness gap from Phase 5 (sealed `DmxController`) prevents us from asserting controller transmissions without a stub; the fix is likely easiest to land alongside Phase 5's test-controller stub.

Out-of-scope workaround: authors can add a no-op effect (e.g. `SineWave` with `min=max=baseline`) to any cue that's currently "assignments only" until the fix lands.
...
**Next actions**:
1. **Fix the Layer 3 transmit gap** (see §"Known issues"). Highest priority — this blocks the Phase 1 verification bullet "PATCH a cue with propertyAssignments via REST client → re-fetching returns the same payload" from being end-to-end-true. The "PATCH and re-fetch" part works; the "stage reflects the assignment" part doesn't.
2. Crossfade-weight integration (Phase 1b): the `// TODO Phase 1b` marker in `buildLayer3AssignmentsForCue` hardcodes `fadeWeight = 1.0`. Teach `CueStackManager.runCrossfade` to push fade progress into `FxEngine.setCueAssignments(…)` so outgoing and incoming cues' Layer 3 contributions fade symmetrically (outgoing cue's assignments currently snap off the moment the incoming cue starts — see `CueStackManager.activateCueInStack` where `removeCueAssignments(oldCueId)` runs before the crossfade job).
3. Remaining `cueEdit.*` follow-up messages: `setPalette` + `addPresetApplication` + `addAdHocEffect`. Stubs exist but reply "not implemented yet". These all need UI to exercise them meaningfully, so land them alongside Phase 2.
4. Live stack-cue edit support. Current `beginEdit` / `setMode` reject cues with a non-null `cueStackId` when mode=LIVE. Next pass: delegate to `CueStackManager.activateCueInStack` and plumb stack deactivation through `endSessionOnDisconnect`.
...
6. Frontend: thread `fixtureKey` through to fixture-level colour components (or accept the Phase 2 hand-off) — see §"Known issues" second item.
7. Second smoke-check pass after the Layer 3 transmit fix: specificity (group red + fixture green override), stop-releases-to-0, stack snap vs crossfade. The framework is now in place (test project "43!", group `hexes`, `/tmp/smoke_helper.py` for WS channel sampling); once the transmit fix lands these should be mechanical.
8. Smoke-check from Phase 0 that's still pending: SineWave + `updateChannel=180`, park+effect, two-effects-same-property priority determinism (fold into #7).
