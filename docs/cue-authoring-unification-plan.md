...
**Phase**: 1 — in flight. Backend DB + DTO + route round-trip landed 2026-04-19a. Layer-3 apply integration + stomp switch landed 2026-04-19b. cueEdit sockets + legacy-effect migration + snapshot-from-live landed 2026-04-19c. Frontend routing, crossfade-weight integration, and the live-rig smoke-check still pending.

**Most recent session**: 2026-04-19c. Three deliverables landed (manual smoke-check deferred — see §"Deviations"):

1. **Legacy static-effect migration.** New `LegacyStaticEffectMigration` in `src/main/kotlin/uk/me/cormack/lighting7/models/MigrateLegacyStaticEffects.kt` with a pure `convertRow(LegacyRow) → ConversionResult` and a DB-touching `run(Transaction) → Summary`. Wired into `State.configureDatabase()` alongside the other startup migrations — reads `cue_ad_hoc_effects` rows with `effect_type` in `{StaticValue, StaticSetting}`, converts each (pulling `parameters["value"]` for `StaticValue` and `parameters["level"]` for `StaticSetting`), INSERTs a matching `cue_property_assignments` row, DELETEs the original. Lossy cases (null property_name, missing value key, non-UByte) log at WARN and are left in place. Idempotent — next startup it's a no-op. 11 new unit tests in `LegacyStaticEffectMigrationTest`.
...
**Next actions**:
1. **Manual smoke check — priority for next session.** Start the backend (lighting7) + frontend (lighting-react) and drive the golden-path flow by hand per the 2026-04-19c session brief: create a cue with one propertyAssignment `(group "front-wash", dimmer=180)`, apply, confirm fixtures hold ~180 with zero active effects; add a SineWave ADDITIVE on the same group's dimmer with amplitude ≈40 and confirm output wiggles in the 180–220 band (this is the headline Layer 2 → Layer 3 reset-to-layer-below behaviour); stop the cue and confirm fixtures release to 0 with the SineWave still running; stack-case + specificity cases per the brief. If any case fails, add a failing test that reproduces before fixing — the component tests did not catch whatever broke.
...
3. `cueEdit.*` follow-up messages: `setMode` mid-session transition (Live↔Blind), `setPalette` + `addPresetApplication` + `addAdHocEffect` + `clearAssignment`. Stubs exist but reply "not implemented yet".
4. Live stack-cue edit support. Current `beginEdit` rejects cues with a non-null `cueStackId` when mode=LIVE. Next pass: delegate to `CueStackManager.activateCueInStack` and plumb stack deactivation through `endSessionOnDisconnect`.
...
6. Frontend (lighting-react): `EditorContext` + routed hooks; extend `cuesApi.ts` / `cues.ts` store types with `propertyAssignments`; add `cueEdit.*` message types.
7. Smoke-check from Phase 0 that's still pending: SineWave + `updateChannel=180`, park+effect, two-effects-same-property priority determinism (fold into #1).
...
| 1 | `CuePropertyAssignment` model + migration; frontend `EditorContext` routing layer | In flight (backend DB + DTO + routes + Layer 3 apply + stomp switch + cueEdit core sockets + legacy migration + snapshot-from-live done; frontend + crossfade-weight + stack-cue edit + cueEdit follow-up messages + live-rig smoke-check still pending) |
