...
<div class="grid2">
  ...
  <div class="col">
    ...
    <div class="note">
      <p><b>D18 is revisited: a window always says which selection it is on</b> (decision 8). While following, a small <b>link badge</b> sits beside the family pill — glyph only, hover <i>Following the desk selection</i>. While local, the dashed <i>Targets: This window</i> chip takes its place and presses back. Same rule as the page's badge. <b>The badge presses too</b> (decision 9): a press takes a selection of this window's own, so the one control on the row flips both ways.</p>
      <p>Where: the rig row in Split and Rig, the <b>pad row in Pads</b> (always following now, so always the badge — drawn as a mark there, its hover saying why, since Pads cannot leave), the compact strip, and the Programmer's row C. Both rows are tight: it is a glyph at every width, measured into their ladders.</p>
      ...
      <p><b>The way in is the badge's own press</b> (decision 9), with the Screens row (below) and ⌘K as the doors from another window; the chip is the way back. ⌘K's <i>Stop following…</i> is withheld in Rig and Pads, where it would be refused anyway.</p>
    </div>
    <div class="note warn">
...
<div class="grid2">
  ...
  <div class="col">
    ...
    <div class="col" style="gap:8px;">
      ...
      <p class="cap"><b>Always drawn while the window is paged with the desk</b> — a small link badge beside the tabs, so a linked window is never silent about it. It names any other open window paged with it (<i>⛓ Screen 2</i>), folding to the glyph alone on the row's ladder; the hover says a tab click pages them too. <b>Pressing the badge</b> keeps the page on show as this window's own (decision 9), and <b>on its own page</b> the badge gives way to the <i>Page: Own</i> chip, which presses back — one control, both ways. The page is always marked one way or the other, which revisits D18 for the page: a small badge, not the old full chip. The glyph is a character here; the app uses lucide's.</p>
    </div>
    <div class="note">
...
<div class="row" style="gap:32px; align-items:flex-start;">
  ...
  <div class="col" style="flex:1; gap:20px;">
    <div class="col" style="gap:8px;">
      ...
      <p class="cap">Top: following — the link badge beside the family pill, whose press gives this window a selection of its own (decision 9). Bottom: local — the dashed chip, whose press follows the desk again. In Pads and Rig only the badge is ever drawn, because those always follow, and there it is a mark rather than a press.</p>
    </div>
    <div class="col" style="gap:8px;">
      <div class="lbl">⌘K</div>
      <div class="cmd">
...
<div class="col">
  ...
  <table>
    ...
    <tr><td class="name">9</td><td>Should the chip itself toggle the two modes? <span class="badge">after session 3</span></td><td class="yes">Yes — press the badge to leave, the chip to come back</td><td>Chris, trying session 3: the chip on the row is what an operator reaches for first, and the Screens sheet is for setting <i>another</i> window. Revises decisions 4 and 8's "a mark, never a control". Where the window cannot leave — the selection in Rig and Pads — the badge stays a mark whose hover says why.</td></tr>
  </table>
</div>
"""
