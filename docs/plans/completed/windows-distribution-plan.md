...
| 2 — Self-contained runtime (fat JAR, child compiler server, mDNS, tray launcher) | ✅ Build-side complete | All Gradle + JVM work landed, build / test / shadowJar / launcher subproject all green; smoke-verified backend boots from `lighting7.jar` and mDNS registers as `lighting7-<host>`. End-to-end install validation tracked in [`followups.md`](../followups.md) → `FU-MANUAL-DIST-INSTALL`. |
| 3 — Packaging (jlink + jpackage, Win + Mac installers) | ✅ Build-side complete | `buildRuntime`, `stageJpackageInput`, `packageMac`, `packageWindows` all wired up. `packageMac` produced `build/installers/lighting7-1.0.0.pkg` (337 MB); trimmed Mac runtime (`build/runtime-mac/`, 59 MB) boots `lighting7.jar` cleanly. Real installer icons (`FU-DIST-ICONS`) and end-to-end install validation (`FU-MANUAL-DIST-INSTALL`) tracked in [`followups.md`](../followups.md). |
