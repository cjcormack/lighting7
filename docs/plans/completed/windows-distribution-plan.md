...
| 2 — Self-contained runtime (fat JAR, child compiler server, mDNS, tray launcher) | ✅ Complete | All Gradle + JVM work landed, build / test / shadowJar / launcher subproject all green; smoke-verified backend boots from `lighting7.jar` and mDNS registers as `lighting7-<host>`. End-to-end install validation passed on Mac + Windows 2026-08-19 — `FU-MANUAL-DIST-INSTALL` in [`manual-validation.md`](../manual-validation.md). |
| 3 — Packaging (jlink + jpackage, Win + Mac installers) | ✅ Complete | `buildRuntime`, `stageJpackageInput`, `packageMac`, `packageWindows` all wired up. `packageMac` produced `build/installers/lighting7-1.0.0.pkg` (337 MB); trimmed Mac runtime (`build/runtime-mac/`, 59 MB) boots `lighting7.jar` cleanly. End-to-end install validation passed on both OSes 2026-08-19 (`FU-MANUAL-DIST-INSTALL` in [`manual-validation.md`](../manual-validation.md)). Real installer icons are still outstanding — `FU-DIST-ICONS` in [`followups.md`](../followups.md). |
...
- [x] New Gradle task `buildRuntime` — uses `jlink` to produce a minimal JRE for the host OS. *(Skipped jdeps auto-discovery: jdeps doesn't walk Spring Boot's BOOT-INF nested-jar layout in the kotlin-compiler-server fat jar, so any auto-discovery still has to be merged with a generous safety baseline — keeping the build deterministic was worth more than a few MB of trim. Final module list: `java.se,jdk.crypto.ec,jdk.unsupported,jdk.zipfs,jdk.localedata`.)*
- [x] Output: `build/runtime-<os>/` (`<os>` derived from `OperatingSystem.current()`).
- [x] Smoke test: trimmed runtime runs `lighting7.jar` end-to-end — schema check, FX loader, ArtNet, mDNS, all 28 effects loaded; `Application started in 12.48 seconds`. Compressed runtime is **59 MB**.

### 3.2 jpackage tasks

- [x] New Gradle task `packageWindows` — calls `jpackage --type msi`, `--name lighting7`, `--app-version <jpackageAppVersion>`, `--vendor "Chris Cormack"`, `--main-jar launcher.jar --main-class uk.me.cormack.lighting7.launcher.LauncherMainKt`, `--input build/jpackage-input/`, `--runtime-image build/runtime-windows/`, `--win-menu --win-shortcut --win-dir-chooser`, optional `--icon assets/lighting7.ico` if present. Gated on `OperatingSystem.current().isWindows`.
- [x] New Gradle task `packageMac` — same shape, `--type pkg`, optional `--icon assets/lighting7.icns` if present. Code signing intentionally out of scope; first-launch Gatekeeper warning is expected. Gated on `OperatingSystem.current().isMacOsX`.
- [x] Both tasks depend on `shadowJar`, `:launcher:shadowJar`, `assembleCompilerServer`, `buildRuntime`, plus a `stageJpackageInput` task that gathers `launcher.jar` + `lighting7.jar` + `kotlin-compiler-server.jar` in `build/jpackage-input/`. Decoupled the installer version from `project.version` (jpackage rejects `0.0.x` because the major must be ≥ 1) — `jpackageAppVersion` Gradle property defaults to `1.0.0`, override via `-PjpackageAppVersion=...` for real releases.
- [x] Build matrix: Windows installer must be built on Windows; Mac installer on Mac. The wrong-host case warns and skips (`onlyIf`).
- [x] Launcher first-launch defaults: `LauncherMain.ensureDefaultConfig()` writes `default-local.conf` (baked into `launcher.jar` resources) to `<appDataDir>/local.conf` if missing, so a fresh install boots without staging files manually.
- [ ] **Icons not shipped** — `assets/lighting7.icns` and `assets/lighting7.ico` don't exist yet; jpackage falls back to its default icon. Generating real icon assets is a follow-up.
...
- [x] **Build-side**: `./gradlew packageMac` produces `build/installers/lighting7-1.0.0.pkg`. `pkgutil --expand` confirms the install layout: `lighting7.app/Contents/MacOS/lighting7` + `Contents/app/{launcher.jar,lighting7.jar,kotlin-compiler-server.jar,lighting7.cfg}` + `Contents/runtime/Contents/Home/` (the trimmed JRE). 337 MB total.
- [ ] **Mac install + launch**: install the `.pkg` on a fresh Mac VM (or wipe `/Applications/lighting7.app` + `~/Library/Application Support/lighting7` first). Double-click → tray icon appears, browser opens, iPad on Wi-Fi reaches `http://lighting7-<hostname>.local:8413/`. Quit from tray → no `java` processes left. *(Manual; deferred to user.)*
- [ ] **Windows**: `./gradlew packageWindows` on a Windows machine → install `.msi` on a clean Windows VM with no JDK. Same checks. Especially verify: `%APPDATA%\lighting7\` permissions, mDNS works on Windows (Bonjour service ships with iTunes/Apple Software Update; if not present on a clean Windows box, JmDNS should still serve names since the iPad does the resolving — but verify), Windows Defender doesn't quarantine the executable. *(Manual; deferred to user. Requires a Windows host.)*
- [ ] **Smoke checklist**: BPM tap, fixture patch CRUD, run a cue, edit a script in the embedded editor (covers compiler server child process), iPad WebSocket reconnects after a brief Wi-Fi drop. *(Manual; deferred to user.)*
