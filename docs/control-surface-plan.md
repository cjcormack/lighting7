...
**Most recent session**: 2026-04-19 (cont.). Phase 4 feedback & reconciliation landed.
...
1. Manually validate Phase 4 on the X-Touch Compact: bind a fader → `front-wash.dimmer`, move the fader, confirm motor drives to the logical value under cue fades. Hold the fader during a cue fade — motor should stop, logical state still updates, motor catches up on release. Bind the `Blackout` button, toggle it and confirm the LED lights up. Bind a button to `Flash` on a group, press and hold — LED on, release — LED off.
2. Start Phase 5 (Frontend `/surfaces` route): device list, binding matrix, MIDI Learn overlay, bank switcher. Phase 5 lives in `lighting-react`, not here. Subscribe to `surfacePickup.changed` for the pickup indicator added this session.
