 ...
 *
 * A slot holds **exactly one** of [cue] / [cueStack] / [look]. The overlay has no selection, so it
 * can hold only what needs none: a cue, a stack, or a Look with no deferred effect — its rows are
 * always bound, so a Look slot presses the Look onto its own fixtures (`docs/plans/busk-layout-plan.md`
 * D7). The Look arm is the busk-layout plan's addition; the stack arm is on its way out with the
 * overlay's own assign flow (session 3) and stays only so the shipped panel keeps working between
 * sessions.
 *
 * The rule is enforced by the routes and the importer, not the schema: the two `CASCADE` options
 * below are not honoured by SQLite without a per-connection pragma (see `templates.group_id`), so
 * a cue's delete sweeps its slots itself, and `look_id` carries no option at all.
 */
