...
object SelectionWrites {
    fun forTargets(
        ...
        // A hue is written onto each head's own current colour, so the write reads the show.
        ...
        for (target in targets) {
            when (TargetRef.ofOrNull(target.type, target.key)) {
                is TargetRef.Fixture -> {
                    ...
                    write(fixture, propertyName, midiValue7Bit, read, sourceGroup = null)?.let(writes::add)
                }
                is TargetRef.Group -> {
                    ...
                    for (member in group.fixtures.filterIsInstance<Fixture>()) {
                        ...
                        write(member, propertyName, midiValue7Bit, read, sourceGroup = target.key)?.let(writes::add)
                    }
                }
                null -> continue
            }
    ...
    private fun write(
        ...
        PropertyChannelResolver.toPropertyValue(fixture, propertyName, midiValue7Bit, read)?.let {
