...
object SelectionWrites {
        ...
        for (target in targets) {
            when (TargetRef.ofOrNull(target.type, target.key)) {
                is TargetRef.Fixture -> {
                    ...
                    write(fixture, propertyName, midiValue7Bit, sourceGroup = null)?.let(writes::add)
                }
                is TargetRef.Group -> {
                    ...
                    for (member in group.fixtures.filterIsInstance<Fixture>()) {
                        ...
                        write(member, propertyName, midiValue7Bit, sourceGroup = target.key)?.let(writes::add)
                    }
                }
                null -> continue
            }
    ...
    private fun write(
        ...
        PropertyChannelResolver.toPropertyValue(fixture, propertyName, midiValue7Bit)?.let {
