...
/**
 * Tests for [captureCueAssignmentsFromSnapshot] — the pure group-shape-preserving collapse
 ...
 */
class CaptureCueAssignmentsTest {
    ...
    private fun slider(v: UByte) = CueAssignmentResolver.PropertyValue.Slider(v)

    @Test
    fun `no group hints - all rows emit as fixture`() {
        val fixtures = fixturesWithTwoHexesInAGroup()
        val snapshot = mapOf(
            CueAssignmentResolver.Key.fixture("hex-1", "dimmer") to slider(100u),
            CueAssignmentResolver.Key.fixture("hex-2", "dimmer") to slider(200u),
        )
        val out = captureCueAssignmentsFromSnapshot(snapshot, emptySet(), fixtures)
    ...
    fun `group hint with uniform member values collapses to a single group row`() {
        ...
        val snapshot = mapOf(
            ...
            CueAssignmentResolver.Key.fixture("hex-2", "dimmer") to slider(150u),
        )
        val hints = setOf("front-wash" to "dimmer")
        val out = captureCueAssignmentsFromSnapshot(snapshot, hints, fixtures)
    ...
    fun `group hint with broken uniformity falls back to per-fixture rows`() {
        ...
        val fixtures = fixturesWithTwoHexesInAGroup()
        val snapshot = mapOf(
            CueAssignmentResolver.Key.fixture("hex-1", "dimmer") to slider(50u),
            CueAssignmentResolver.Key.fixture("hex-2", "dimmer") to slider(150u),
        )
        val hints = setOf("front-wash" to "dimmer")
        val out = captureCueAssignmentsFromSnapshot(snapshot, hints, fixtures)
    ...
    fun `group collapse plus unrelated fixture row round-trips both`() {
        ...
        val fixtures = fixturesWithTwoHexesInAGroup()
        val snapshot = mapOf(
            CueAssignmentResolver.Key.fixture("hex-1", "dimmer") to slider(200u),
            CueAssignmentResolver.Key.fixture("hex-2", "dimmer") to slider(200u),
            CueAssignmentResolver.Key.fixture("hex-1", "uv") to slider(50u),
        )
        val hints = setOf("front-wash" to "dimmer")
        val out = captureCueAssignmentsFromSnapshot(snapshot, hints, fixtures)
        assertEquals(2, out.size)
    ...
    fun `empty snapshot emits empty output regardless of hints`() {
        val fixtures = fixturesWithTwoHexesInAGroup()
        val out = captureCueAssignmentsFromSnapshot(
            emptyMap(),
    ...
    fun `sortOrder is dense zero-indexed`() {
        ...
        val snapshot = mapOf(
            CueAssignmentResolver.Key.fixture("hex-1", "dimmer") to slider(100u),
            CueAssignmentResolver.Key.fixture("hex-2", "dimmer") to slider(100u),
            CueAssignmentResolver.Key.fixture("hex-1", "uv") to slider(50u),
        )
        val hints = setOf("front-wash" to "dimmer")
        val out = captureCueAssignmentsFromSnapshot(snapshot, hints, fixtures)
