...
- `selection.state` / `selection.set` / `selection.toggle` / `selection.clear` - The desk's one shared selection (`state/DeskSelection.kt`): what a selection-relative surface control and a busk press act on. `StateFlow`-backed, so the subscription is the connect snapshot; writes reply nothing (a no-op write sends no frame). `toggle` is head-by-head: a group all of whose members are selected is "in", and toggling it off narrows the entries that covered it
