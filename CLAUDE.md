...
- `windows.viewOptions` `{targetId, view, options}` - One generic command for a window's per-view options (busk `focus`, `rigRows`, `sheet`), rebroadcast like the other four; the target applies them for that view only and re-announces. `windows.announce` carries `viewOptions?` (absent from an older client) and `windows.state` carries it back
